...
def test_training_mlp():
    ...
    kmodel.add(tf.keras.layers.Dense(64, activation="tanh"))
    kmodel.add(tf.keras.layers.Dense(64, activation="softmax"))
    kmodel.add(tf.keras.layers.Dense(y.shape[1], activation="sigmoid"))
    ...
    model.fit(
        X, y, "categorical_crossentropy", epochs=EPOCHS, batch_size=BATCH_SIZE,
        learning_rate=LEARNING_RATE)
