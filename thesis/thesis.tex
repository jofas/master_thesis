...
at handling lots of packets, the supported peek of packets being on
...
If all vertices send at the same time, the peek amount of packets on
the communication fabric is obviously high.
With the timer offset one can distribute the peek across time.
...
% also don't forget the offset between sending successive packets
...
%
%Extra cores can be enabled to decrease loading time before the
%execution starts and for packet re-injection of dropped packets, which
%decreases the number of cores per chip which can be used for the deep
%learning model \citep{furber_et_al_2020}.
%
