...
The quintessential artificial neural network is the multilayer
perceptron (MLP), also called a feedforward neural network
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
\caption {Schema of a MLP or feedforward neural network. The grey
  edges are directed edges from left to right.}
\label{fig:mlp}
\end{figure}
...
\begin{align}
  ...
  g(\mathbf{x}^\top \mathbf{W} + \mathbf{b}).
\end{align}
...
It is an algorithm that allows to train a deep learning model with
\textit{(stochastic or batch) gradient descent}.
...
The same procedure is applied to the following hidden layers, only
that the loss is now computed as:
\begin{align}
  L^{(l-1)} = \sum_{i=1}^{k}\frac{\delta L}{\delta h_k},
\end{align}
which can be interpreted as the sum over the error of each perceptron
in the previous layer.

\citet{hornik_et_al_1989} demonstrated that a non-linear MLP
(the activation functions are non-linear transformations of
$\mathbf{x}^\top \cdot \mathbf{W} + \mathbf{b}$) can overcome the
...
training bigger models (more hidden layers---the depth of the neural
networks increased) which can solve more complex problems
