...
\footnote{Or (batch) gradient descent. With gradient descend the whole
  ...
  training set multiple times to the model. Each pass over the whole
  training set is called an \textit{epoch}.
}:
\begin{align}
...
$\mathbf{x}^\top \mathbf{W} + \mathbf{b}$) can overcome the
...
\textit{kernel} \citep{goodfellow_et_al_2016}.
...
\begin{tikzpicture}[box/.style={rectangle,draw, minimum size=.5cm}]
  ...
  \node[right=.1 of fst, box] (snd) {$bx + cy + dz$};
...
\caption {Schema for the convolutional layer performing the
  convolution shown in Figure~\ref{fig:conv_op}. The schema shows the
  property of sparse connectivity and shared weights. The black edges
...
\begin{enumerate}
  \item filters, kernels and channels
  \item shared weights sparsity, spatial invariance
