...
% TODO: batch gradient descent as equation in intro to dl
    ...
    \FOR{1,\dots,epochs}
      \FOR{$\mathbf{x}_i \in \mathbf{X}, i=1,\dots,|\mathbf{X}|$}
        ...
        \IF{$i \mod$ batch size == 0 || epoch complete}
          \STATE{update weights with some form of (\ref{eq:sgd})}
          \COMMENT{depending on the batch size (gradients as sum over
            $\frac{L_i}{w_{jk}}$)}
        \ENDIF
