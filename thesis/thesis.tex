...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}[block/.style={rectangle, draw, minimum width=6cm}]
      ...
      \node[below=.2 of block15] (pool2) {Avg pool};
      \node[block, below=.2 of pool2] (dense) {dense 1000};

      \node[above=.2 of block0] (pool1) {Max pool, /2};
...
Volta \citep{durant_et_al_2017}, with the successor Ampere released in
2020, which, according to NVIDIA, is much more powerful
\citep{krashinsky_et_al_2020}.
...
put more focus on SpiNNaker's massive parallelism rather than relying
on fast instruction level parallelism.
