...
periodic update event, called every $x$ cycles.
...
The same goes for the discovered bug during the key allocation
...
Both, the interface change and the bug together, was solved in
a single day.

Not really an issue during development but during testing was the
...
reuse the socket.
While this removed the error, unfortunately it led to inconsistent
behavior of the second live IO session.
...
While this has been an inconvenience, the overall time lost by trying
out the workaround described above and for having to execute the test
...
toolchain.
They have led to approximately two weeks of lost time during the
dissertation phase.
...
Like stated in Section~\ref{subsec:SpiDNN_arch}, the time and effort
...
Too much time was wasted by trying to be fast.
Convolutional layers were implemented after forward and backward pass
of the MLP.
With already one and two weeks behind schedule precious time was
wasted by having to debug cases, where padding and offset were
computed wrongfully.
Getting the formula for computing the offset for strided convolutional
layers with ``same'' padding right took another week.
...
The problem with strides arise during the backward pass, where we
...
During data generation (see Section~\ref{subsec:spinn_toolchain}),
the neuron gets the information of its placement from the toolchain.
A maximum timer offset is provided by the prototype.
It is the time between update events, divided by ten.
90 percent of the time between update events should leave enough time
...
$n \cdot m$ (bear in mind that only one layer sends at a time, thanks
...
execution starts and for packet re-injection of dropped packets, which
decreases the number of cores per chip down to 15
\citep{furber_et_al_2020}.
...
There were three things tried in order to solve the problem:
...
$m$ keys for all the outgoing edge partitions of the layer.
...
entries to fit into the routing table, even for small networks.
...
searching for a solution with minimal packets.
...
computational effort of the backward pass is doubled.
...
When it became clear we were not able to finish our implementation
with shared parameters, we changed our approach for communication
during the backward pass again, trying to tackle the problem arising
from the complexity of the code.
% transition to backward pass ideas

% and the time spend in receive... god, all the time spend in
% receive

% then the big packet thing

% here mention what features are missing for ResNet

% then the problems of the design in the first place
